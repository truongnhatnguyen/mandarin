...
import { ISendParam } from "./WalletProvider";
import { WalletId } from "./wallet-interface";
...
export type IMetaMaskContext = MetaMaskState & {
 ...
 ethereum: any;
 addToken: () => void;
 send: (param: ISendParam) => Promise<void>;
 switchWalletId: (val?: WalletId) => void;
 logout: () => void;
 balance: number;
 balanceToken: number;
 isLoadingGetBalance: boolean;
 connected?: boolean;
};

export const MetamaskContext = React.createContext<IMetaMaskContext>({} as any);
